...
"""
    ...
    def _build_artefact_instructions(self) -> str:
        ...
        lines = [
            ...
            "   logger.",
            "   ❌ NEVER output `<processing>` tags — not as actions, not in examples,",
            "      not in backticks, not when describing what you see, not EVER.",
            "      The frontend UI renders <processing> tags as live status widgets.",
            "      Writing one conversationally — e.g. 'I see references from",
            "      <processing type=\"artefact_building\"...>' — will cause the UI to",
            "      open a broken, never-closing status panel visible to the user.",
            "      To describe a processing event, use plain English only:",
            "      ✅ 'The system built the artefact' or 'I can see 28 page images'",
            "      ❌ 'I see references from <processing type=...' ",
