...
"""
    ...
    def _build_artefact_instructions(self) -> str:
        ...
        lines = [
            ...
            "6. TERMINATION PROTOCOL (MANDATORY): When all objectives are reached, or if the user's request is a simple conversational/capability question requiring no further tool or file actions, you MUST output `<Done />` on a brand-new line at the very end of your response to signal completion.",
            "7. STRICT THINKING SEGREGATION (ABSOLUTE RULE): The thinking phase (<think>...</think>) is solely for planning and cognitive reasoning. You are STRICTLY FORBIDDEN from generating `<processing>`, `</processing>`, `<lollms_event>`, `<tool_call>`, or `<tool_result>` tags inside your thoughts. You must NEVER simulate mock execution outcomes or write fake terminal logs. Those tags are strictly generated by the system runner.",
            "",
            ...
            "",
            "=== TWO-STEP PROTOCOL ===",
            "Step 1: <coding_plan> (MANDATORY before <artifact>)",
            "<coding_plan>",
            "  - Goal: [summary]",
            "  - Specialist Persona: [expertise]",
            "  - Target Artifacts: [names]",
            "  - Implementation Details: [steps]",
            "  - Supporting Context: [reference docs]",
            "  - Relevant Skills: [skill titles]",
            "  - Relevant Memories: [8-char IDs]",
            "</coding_plan>",
            "🚨 NEVER use <coding_plan>/<artifact> for read-only data queries.",
            "",
            "Step 2: EXECUTION",
            "CREATE: <artifact name=\"file.ext\" type=\"type\">full content</artifact>",
            "PATCH (PREFERRED): <artifact name=\"file.ext\">",
            "<<<<<<< SEARCH",
            "exact verbatim lines (incl. indentation)",
            "=======",
            "replacement lines",
            ">>>>>>> REPLACE",
            "</artifact>",
            "RENAME: <artifact name=\"new.ext\" rename=\"old.ext\">content</artifact>",
            "REVERT: <revert_artifact name=\"file.ext\" version=\"3\" />",
            "EPHEMERAL: <artifact name=\"query.py\" ephemeral=\"true\">raw code</artifact> → pass name to tool",
            "",
