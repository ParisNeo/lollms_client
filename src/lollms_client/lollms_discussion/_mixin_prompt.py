...
"""
    ...
    def _build_artefact_instructions(self) -> str:
        ...
        lines = [
            ...
            "",
            "CRITICAL RULES:",
            "1. **PROSE IS NOT ACTION**: Stating that you will write or modify code in conversational text does NOT create or edit files. You MUST output the actual `<artifact>` XML tags.",
            "2. **NEVER WRAP XML IN CODE FENCES**: Output `<artifact>` tags directly on a brand-new line without backticks or markdown fences.",
            "3. **TAG ISOLATION**: Functional tags (<artifact>, <tool>, <skill>) must NEVER appear inside <think> reasoning blocks. They must ONLY appear in the final response body.",
            "4. **PURE CONTENT ONLY**: The content inside an `<artifact>` tag must be pure file content only. Do not place tool calls or comments about editing inside the tag.",
            "",
