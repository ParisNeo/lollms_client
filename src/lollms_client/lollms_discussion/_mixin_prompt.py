...
"""
    ...
    def _build_artefact_instructions(self) -> str:
        ...
        lines = [
            ...
            "• Markdown = conversational display only",
            "• Artifacts = persistent, saved files in the workspace",
            "",
            "CRITICAL RULES:",
            "1. **PROSE IS NOT ACTION (SAME-RESPONSE MANDATE)**: Stating that you will write or modify code in conversational text (in ANY language) does NOT create or edit files. You MUST output the actual `<artifact>` XML tags IN THE SAME RESPONSE immediately after your brief introduction.",
            "2. **NEVER SPLIT INTENT AND EXECUTION**: Never announce what you will build and then stop without outputting the tag. The tag must appear in the same turn.",
            "3. **NEVER WRAP XML IN CODE FENCES**: Output `<artifact>` tags directly on a brand-new line without backticks or markdown fences.",
            "4. **TAG ISOLATION**: Functional tags (<artifact>, <tool>, <skill>) must NEVER appear inside <think> reasoning blocks. They must ONLY appear in the final response body.",
            "5. **PURE CONTENT ONLY**: The content inside an `<artifact>` tag must be pure file content only. Do not place tool calls or comments about editing inside the tag.",
            "",
            ...
            "",
            "=== TOOL FAILURE RECOVERY PROTOCOL (MANDATORY) ===",
            "When a tool call returns an error or fails, you MUST adapt — never repeat blindly:",
            "1. **NEVER RETRY AN IDENTICAL CALL**: Calling the same tool with the exact same parameters that just failed is FORBIDDEN. It will fail again and wastes a round.",
            "2. **ANALYZE BEFORE RETRYING**: Read the error message carefully. Identify the cause (wrong name, missing project, unloaded resource, malformed syntax) and fix THAT specific cause in your next call.",
            "3. **SWITCH STRATEGY**: If the same goal fails twice with corrected parameters, switch to a DIFFERENT tool or approach entirely (e.g. if an ontology project fails to load, use tool_list_ontology_projects to verify the exact project id, or operate on the workspace file directly).",
            "4. **ESCALATE AFTER 2 FAILURES**: After two consecutive failed attempts on the same goal, STOP and report the blocker to the user with what you tried and what you need from them. Do not loop.",
            "5. **VERIFY RESOURCE IDENTIFIERS**: Before calling a tool that takes a project/file/resource name, confirm the identifier exists (use a listing tool first if unsure).",
            "=== END TOOL FAILURE RECOVERY PROTOCOL ===",
            "",
            ...
            "",
            "=== SUB-AGENT SPAWNING (<agent> tag) ===",
            "To delegate complex, heavy, or self-contained tasks to a focused sub-agent, emit an `<agent>` functional tag.",
            "It must start on a new line and never be wrapped in code fences.",
            "Define the specialization dynamically via the `system_prompt` attribute (e.g. system_prompt=\"You are an expert Python data engineer...\").",
            "The sub-agent will run in the same engine, stream its work to the user live, save all generated files to disk,",
            "and return a verified report to you as `[WORKER REPORT]`.",
            "",
            "🚨 STRICT TAG GRAMMAR (MANDATORY — parser rejects malformed blocks):",
            "1. The task body MUST be wrapped in `<task>` ... `</task>` XML tags. NO exceptions.",
            "2. ❌ FORBIDDEN: plain-text wrappers like `=== TASK ===` / `=== END TASK ===` inside `<agent>` blocks. The parser treats them as a missing task and REJECTS the entire spawn.",
            "3. `<context_files>` is optional; when present, it must be a valid XML block listing one existing filename per line.",
            "4. Emit at most ONE `<agent>` block per response. The parser accepts exactly one delegation per turn.",
            "5. Do not nest any other functional tags inside the `<agent>` block — only `<task>` and optionally `<context_files>`.",
            "",
            "✅ CORRECT EXAMPLE (copy this structure exactly):",
            '<agent name="data_engineer" system_prompt="You are an expert Python data engineer." max_rounds="8">',
            "<task>",
            ...
            "</agent>",
            "",
            "❌ WRONG EXAMPLE (this WILL be rejected — plain-text wrapper instead of <task> XML):",
            "<agent name=\"data_engineer\">",
            "=== TASK ===",
            "Build the ontology file.",
            "=== END TASK ===",
            "</agent>",
            "",
    ...
    def _build_note_instructions(self) -> str:
        lines = [
            ...
            "",
            "To CREATE a new note, ALWAYS output the actual `<note title=\"...\">` XML tag in the SAME response where you announce the note:",
    ...
    def _build_skill_instructions(self) -> str:
        lines = [
            ...
            "",
            "To CREATE a new skill, ALWAYS output the actual `<skill title=\"...\" description=\"...\" category=\"...\">` XML tag in the SAME response where you announce the skill. Stating in prose that you will create a skill without emitting the tag creates nothing:",
